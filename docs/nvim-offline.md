...
- 상태줄 오른쪽 파일타입 옆: 연결된 서버가 있으면 `[LSP O: ty]`, 없으면 배경색을 강조한
  `[LSP X]`를 표시합니다. 여러 서버는 이름을 쉼표로 구분합니다.
- `[FORMAT O: ruff]`처럼 실제 사용할 외부 포맷터 이름을 표시합니다. LSP 포맷팅을 사용하면 서버 이름을 표시하며, 사용할 수 없으면 `[FORMAT X]`를 표시합니다.
...
Sticky Scroll은 기본으로 켜지며 `Space Ts`로 토글합니다. 화면 밖의 함수·조건문·반복문
시작 줄을 들여쓰기 기준으로 최대 5줄 표시하고, 배경색과 아래 구분선으로 본문과 구별합니다.
고정 영역과 구분선 바로 아래에 보이는 본문을 기준으로 시작 줄을 찾으므로, 고정 영역에 가려진 선언도 함께 표시합니다.
줄 번호는 각 고정 줄의 원본 행 번호이며, 구분선에는 번호를 표시하지 않습니다.
본문의 줄 번호·기호 영역 너비와 들여쓰기 가이드, 가로 스크롤 위치를 맞춰 코드 열을 유지합니다.
Python의 여러 줄 함수 선언은 닫는 `):` 대신 `def 함수명(` 줄을 표시합니다.
중첩이 깊어도 최상위 블록과 함수 선언을 우선 남기며, 나머지 칸에는 가까운 내부 블록을 표시합니다.
창이 작아 한 줄만 표시할 수 있을 때는 함수 선언을 우선합니다.
활성 편집창에만 표시하며 netrw·터미널·팝업·대용량 파일에서는 끕니다.
외부 플러그인이나 LSP·파서는 필요하지 않습니다. 위쪽 1,000줄, 조회 구간 256 KiB까지만
탐색하므로 범위 밖의 시작 줄은 생략될 수 있고, 복잡한 여러 줄 구문·주석은 완전히 해석하지 않습니다.
